(ns relativity.main
  "Kotodama WASM entrypoint for the Relativity clean-room actor (L4) — Clojure port.
  ...
  facts (`relativity.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "relativity")
(def tier "L4")
...
(def entity-specs
  [{:entity "Record"   :plural "records"   :id-prefix "relativi_rec"
...
(defn emit-facts
  "EAVT facts for one record: {\"relativity.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "relativity-compat" :tier tier :entities entities} 200])
...
(defn start! [] :relativity-compat/ready)
